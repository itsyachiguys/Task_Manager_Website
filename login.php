<?php // start php code block
session_start(); // start session for user authentication
if ($_SERVER['REQUEST_METHOD'] == 'POST') { // check if form was submitted via post
    $username = $_POST['username']; // get username from form input
    $password = $_POST['password']; // get password from form input
    $users = json_decode(file_get_contents('users.json'), true); // load users from json file
    foreach ($users as $user) { // loop through users to find match
        if ($user['username'] == $username && password_verify($password, $user['password'])) { // verify username and password
            $_SESSION['user'] = $username; // set session variable for logged in user
            header('Location: index.php'); // redirect to home page
            exit; // stop script execution
        }
    }
    $error = 'Invalid username or password'; // set error message if login fails
}
?> // end php code block
<!DOCTYPE html> <!-- declare html5 document type -->
<html lang="en"> <!-- html root element with english language -->
<head> <!-- head section for metadata -->
  <meta charset="utf-8"/> <!-- set character encoding to utf-8 -->
  <meta name="viewport" content="width=device-width,initial-scale=1"/> <!-- responsive viewport meta tag -->
  <title>Login - Pastel Task Manager</title> <!-- page title for browser tab -->
  <link rel="stylesheet" href="style.css"/> <!-- link to external css file -->
  <style> <!-- internal css styles for login page -->
    .login-container {
      min-height: 100vh;
      display: flex;
      align-items: center;
      justify-content: center;
      position: relative;
      overflow: hidden;
    }
    .login-card {
      background: rgba(255, 255, 255, 0.95);
      backdrop-filter: blur(20px);
      border: 1px solid rgba(255, 255, 255, 0.3);
      width: 400px;
      height: 400px;
      padding: 40px;
      border-radius: 24px;
      box-shadow: 0 20px 60px rgba(133, 98, 120, 0.15);
      position: relative;
      z-index: 10;
      animation: slideInUp 0.8s cubic-bezier(0.4, 0, 0.2, 1);
      display: relative;
      flex-direction: column;
      justify-content: center;
      margin-left: 200px;
      margin-right: 100px;
    }
    .login-header {
      text-align: center;
      margin-bottom: 32px;
    }
    .login-icon {
      font-size: 48px;
      margin-bottom: 16px;
      display: block;
    }
    .login-title {
      font-size: 28px;
      font-weight: 700;
      color: #5a2f45;
      margin: 0 0 8px 0;
    }
    .login-subtitle {
      font-size: 14px;
      color: #6b495b;
      margin: 0;
    }
    .form-group {
      margin-bottom: 24px;
      position: relative;
    }
    .form-group label {
      display: block;
      margin-bottom: 8px;
      font-weight: 600;
      color: #5a2f45;
      font-size: 14px;
    }
    .form-group input {
      width: 100%;
      padding: 16px 20px;
      border: 2px solid rgba(0, 0, 0, 0.06);
      border-radius: 16px;
      background: rgba(255, 255, 255, 0.9);
      font-size: 16px;
      color: #4a2c39;
      transition: all 0.3s cubic-bezier(0.4, 0, 0.2, 1);
      backdrop-filter: blur(5px);
    }
    .form-group input:focus {
      outline: none;
      border-color: #ff6b9d;
      box-shadow: 0 0 0 4px rgba(255, 107, 157, 0.2);
      transform: translateY(-2px);
    }
    .form-group input::placeholder {
      color: #8b6b7a;
    }
    .error-message {
      background: linear-gradient(135deg, #ff9a9e, #fecfef);
      color: #8a1a2a;
      padding: 12px 16px;
      border-radius: 12px;
      margin-bottom: 20px;
      font-size: 14px;
      font-weight: 600;
      border: 1px solid rgba(255, 154, 158, 0.3);
      animation: slideInUp 0.6s cubic-bezier(0.4, 0, 0.2, 1);
    }
    .login-btn {
      width: 100%;
      padding: 16px;
      background: linear-gradient(135deg, #ff6b9d, #ff8fab);
      color: #6b2540;
      border: none;
      border-radius: 16px;
      font-size: 16px;
      font-weight: 700;
      cursor: pointer;
      transition: all 0.3s cubic-bezier(0.4, 0, 0.2, 1);
      position: relative;
      overflow: hidden;
      box-shadow: 0 8px 20px rgba(133, 98, 120, 0.12);
    }
    .login-btn:hover {
      transform: translateY(-2px) scale(1.02);
      box-shadow: 0 12px 30px rgba(133, 98, 120, 0.2);
    }
    .login-btn:active {
      transform: translateY(0) scale(0.98);
    }
    .login-btn::before {
      content: '';
      position: absolute;
      top: 0;
      left: -100%;
      width: 100%;
      height: 100%;
      background: linear-gradient(90deg, transparent, rgba(255, 255, 255, 0.3), transparent);
      transition: left 0.6s;
    }
    .login-btn:hover::before {
      left: 100%;
    }
    .demo-info {
      text-align: center;
      margin-top: 24px;
      padding: 16px;
      background: linear-gradient(135deg, rgba(255, 214, 232, 0.6), rgba(198, 231, 217, 0.4));
      border-radius: 12px;
      border: 1px solid rgba(255, 255, 255, 0.8);
    }
    .demo-info h4 {
      margin: 0 0 8px 0;
      color: #5a2f45;
      font-size: 14px;
      font-weight: 700;
    }
    .demo-info p {
      margin: 0;
      font-size: 12px;
      color: #6b495b;
    }
    @keyframes slideInUp {
      from {
        opacity: 0;
        transform: translateY(30px) scale(0.95);
      }
      to {
        opacity: 1;
        transform: translateY(0) scale(1);
      }
    }
  </style> <!-- end of internal css styles -->
</head> <!-- end of head section -->
<body> <!-- body element for page content -->
  <div class="login-container"> <!-- main container for login page -->
    <!-- Animated background -->
    <div style="position: absolute; top: 0; left: 0; width: 100%; height: 100%; background: linear-gradient(180deg, #fff8fb 0%, #fffaf6 50%, #f8f9fa 100%); z-index: -2;"></div> <!-- background gradient layer -->
    <div style="position: absolute; top: 0; left: 0; width: 100%; height: 100%; background: radial-gradient(circle at 20% 20%, rgba(255,107,157,0.15) 0%, transparent 50%), radial-gradient(circle at 80% 80%, rgba(78,205,196,0.12) 0%, transparent 50%), radial-gradient(circle at 40% 60%, rgba(177,156,217,0.1) 0%, transparent 50%); z-index: -1; animation: float 20s ease-in-out infinite;"></div> <!-- animated background orbs -->

    <div class="login-card"> <!-- card container for form elements -->
      <div class="login-header"> <!-- header section with icon and title -->
        <span class="login-icon">🔐</span> <!-- lock icon for login -->
        <h1 class="login-title">Welcome Back</h1> <!-- main heading -->
        <p class="login-subtitle">Sign in to continue your productivity journey</p> <!-- subtitle -->
      </div> <!-- end of header -->

      <?php if (isset($error)): ?> <!-- conditional display of error message -->
        <div class="error-message"> <!-- error message container -->
          <strong>⚠️ Login Failed</strong><br> <!-- error icon and title -->
          <?php echo htmlspecialchars($error); ?> <!-- display sanitized error message -->
        </div> <!-- end of error message -->
      <?php endif; ?> <!-- end of conditional -->

      <form method="post" action="login.php"> <!-- login form with post method -->
        <div class="form-group"> <!-- form group for username input -->
          <label for="username">Username</label> <!-- label for username input -->
          <input type="text" id="username" name="username" placeholder="Enter your username" required> <!-- text input for username -->
        </div> <!-- end of form group -->

        <div class="form-group"> <!-- form group for password input -->
          <label for="password">Password</label> <!-- label for password input -->
          <input type="password" id="password" name="password" placeholder="Enter your password" required> <!-- password input -->
        </div> <!-- end of form group -->

        <button type="submit" class="login-btn">Sign In ✨</button> <!-- submit button -->
      </form> <!-- end of form -->

      <div style="text-align: center; margin-top: 16px;"> <!-- centered container for forgot password link -->
        <a href="forgot_password.php" style="color: #6b2b46; text-decoration: none; font-size: 14px; font-weight: 600;">Forgot Password?</a> <!-- link to forgot password page -->
      </div> <!-- end of link container -->

      <div class="demo-info"> <!-- demo credentials information box -->
        <h4>Demo Credentials</h4> <!-- heading for demo info -->
        <p><strong>Username:</strong> admin<br><strong>Password:</strong> password</p> <!-- demo username and password -->
      </div> <!-- end of demo info -->
    </div> <!-- end of login card -->
  </div> <!-- end of login container -->

  <style> <!-- additional internal styles -->
    @keyframes float { <!-- keyframe animation for floating effect -->
      0%, 100% { transform: translateY(0px) rotate(0deg); } <!-- start and end state -->
      50% { transform: translateY(-10px) rotate(1deg); } <!-- middle state -->
    } <!-- end of keyframe -->
  </style> <!-- end of additional styles -->
</body> <!-- end of body -->
</html> <!-- end of html document -->
