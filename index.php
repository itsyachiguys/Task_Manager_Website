...
<body>
  ...
  <header class="header">
    <div class="brand">
      <div class="logo">✨</div>
      <div>
        <div class="brand-title">Hello, <?php echo htmlspecialchars($_SESSION['user']); ?>!</div>
        <div class="brand-subtitle">Focus • Plan • Bloom</div>
      </div>
      <!-- theme toggle button for light/dark mode switching -->
      <button id="modeToggle" aria-label="Toggle color mode" title="Toggle color mode" style="background:none; border:none; cursor:pointer; font-size:24px; margin-left:12px; color:#6b2b46;">🌓</button>
      <a href="logout.php" style="margin-left:12px; color:#6b2b46; text-decoration: none;">Logout</a>
    </div>
  ...
  <div class="container">
    ...
    <main>
      ...
      <div class="card">
        <div class="h1">📅 Upcoming Deadlines</div>
...
<script>
  ...
  function renderUpcoming(){
    ...
    list.forEach((t,i)=>{
      ...
      info.innerHTML = `<div style="font-weight:700;color:#5a2f45">${t.title}</div>
        <div style="font-size:12px;color:#6b495b">Due: ${t.dueDate} • ${days} day(s)</div>
  ...
  function renderAlerts(){
    ...
    if(overdue.length){
      ...
      alertDiv.innerHTML = `<div class="alert-icon">🚨</div><div class="alert-content"><strong>${overdue.length} task(s) overdue!</strong><div class="alert-subtitle">Take action immediately</div></div><button class="alert-dismiss" onclick="this.parentElement.remove()">✕</button>`;
      area.appendChild(alertDiv);
    }
    // show warning alert for tasks due within 3 days
    if(dueSoon.length){
      const alertDiv = App.el("div","alert warning");
      alertDiv.innerHTML = `<div class="alert-icon">⚠️</div><div class="alert-content"><strong>${dueSoon.length} task(s) due within 3 days</strong><div class="alert-subtitle">Plan your time wisely</div></div><button class="alert-dismiss" onclick="this.parentElement.remove()">✕</button>`;
      area.appendChild(alertDiv);
    }
    // show success alert for completed tasks
    if(completed.length > 0){
      const alertDiv = App.el("div","alert success");
      alertDiv.innerHTML = `<div class="alert-icon">🎉</div><div class="alert-content"><strong>${completed.length} task(s) completed!</strong><div class="alert-subtitle">Great progress! Keep it up</div></div><button class="alert-dismiss" onclick="this.parentElement.remove()">✕</button>`;
      area.appendChild(alertDiv);
    }
    ...
    if(!overdue.length && !dueSoon.length && completed.length === 0){
      ...
      alertDiv.innerHTML = `<div class="alert-icon">✨</div><div class="alert-content"><strong>All caught up!</strong><div class="alert-subtitle">No pressing deadlines</div></div><button class="alert-dismiss" onclick="this.parentElement.remove()">✕</button>`;
      area.appendChild(alertDiv);
    }
  }
